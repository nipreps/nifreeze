...
def simulate_pet_motion(base_dir, orig_sub="sub-01", num_subjects=1, session="ses-baseline"):
    ...
    """
    orig_pet_path = os.path.join(
        base_dir, orig_sub, session, "pet", f"{orig_sub}_{session}_pet.nii.gz"
    )
    orig_json_path = os.path.join(
        base_dir, orig_sub, session, "pet", f"{orig_sub}_{session}_pet.json"
    )
    orig_blood_json = os.path.join(
        base_dir, orig_sub, session, "pet", f"{orig_sub}_{session}_recording-manual_blood.json"
    )
    orig_blood_tsv = os.path.join(
        base_dir, orig_sub, session, "pet", f"{orig_sub}_{session}_recording-manual_blood.tsv"
    )
    orig_anat_path = os.path.join(
        base_dir, orig_sub, session, "anat", f"{orig_sub}_{session}_T1w.nii"
    )

    pet_img = nb.load(orig_pet_path)
    ...
    for subj_idx in range(num_subjects):
        ...
        shutil.copy(
            orig_blood_json,
            os.path.join(new_sub_pet_dir, f"{new_sub}_{session}_recording-manual_blood.json"),
        )
        shutil.copy(
            orig_blood_tsv,
            os.path.join(new_sub_pet_dir, f"{new_sub}_{session}_recording-manual_blood.tsv"),
        )
        ...
        motion_df.to_csv(
            os.path.join(new_sub_pet_dir, f"{new_sub}_{session}_ground_truth_motion.csv"),
            index=False,
        )
...
if __name__ == "__main__":
    base_dir = (
        "/Users/martinnorgaard/Downloads/"
        "eddymotion_pet_testdata/data"
    )  # Update to your dataset root
