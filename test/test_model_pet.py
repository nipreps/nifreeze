...
def test_pet_base_model():
    ...
    with pytest.raises(
        ...
        BasePETModel(None, timepoints=None, xlim=None)  # type: ignore[abstract, arg-type]
...
def test_petmodel_init_dataset_error(request, setup_random_pet_data, monkeypatch):
    ...
    timepoints = rng.random(pet_dataobj.shape[-1])
    ...
    with pytest.raises(TypeError, match=PET_OBJECT_ERROR_MSG):
        BSplinePETModel(dataset=pet_obj_attless, timepoints=timepoints, xlim=xlim)  # type:ignore[arg-type]
    ...
    with pytest.raises(TypeError, match=PET_OBJECT_ERROR_MSG):
        BSplinePETModel(dataset=pet_obj_midf, timepoints=timepoints, xlim=xlim)  # type:ignore[arg-type]
    ...
    with pytest.raises(ValueError, match=PET_MIDFRAME_ERROR_MSG):
        BSplinePETModel(dataset=pet_obj_totald, timepoints=timepoints, xlim=xlim)  # type:ignore[arg-type]
...
def test_petmodel_init_parameters_error(request, setup_random_pet_data, none_params):
    ...
    with pytest.raises(
        ...
        BSplinePETModel(dataset=pet_obj, timepoints=timepoints, xlim=xlim)  # type: ignore[arg-type]
...
def test_petmodel_init_timepoint_value_error(request, setup_random_pet_data):
    ...
    with pytest.raises(
        ...
        BSplinePETModel(dataset=pet_obj, timepoints=timepoints, xlim=xlim)
