...
@pytest.mark.parametrize("attr_name, excess_value",
    [("midframe", 1.0), ("midframe", 2.0), ("total_duration", 0.0), ("total_duration", -1.0), ("total_duration", -2.0)],
)
def test_pet_instantiation_attribute_inconsistency_error(setup_random_pet_data, attr_name, excess_value):
    pet_dataobj, affine, _, _, midframe, total_duration = setup_random_pet_data
...
def test_from_nii_errors(tmp_path, setup_random_pet_data):
    pet_dataobj, affine, brainmask_dataobj, frame_time, midframe, total_duration = setup_random_pet_data
...
def test_from_nii(tmp_path, setup_random_pet_data):
    ...
    pet_fname, brainmask_fname, temporal_fname = _serialize_pet_data(pet, brainmask, frame_time, tmp_path)
