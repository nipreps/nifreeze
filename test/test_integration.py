...
import nifreeze.registration.ants as ants
from nifreeze.data import dmri
from nifreeze.estimator import Estimator
from nifreeze.model.base import MASK_ABSENCE_WARN_MSG, TrivialModel
...
def _noop_advanced_clip(data, *args, **kwargs):
    # Behave like advanced_clip: return an array unless inplace=True
    if kwargs.get("inplace", False):
        return None
    return np.asarray(data)


def _identity_run_registration(*args, **kwargs):
    # Must return a nitransforms BaseTransform (real function returns nt.linear.Affine)
    return nt.linear.Affine(np.eye(4))


def _dwi_data_to_nifti(dwi_dataobj, affine, brainmask_dataobj, b0_dataobj):
...
def _serialize_dwi_data(
    ...
    nb.save(b0, b0_fname)
    np.savetxt(gradients_fname, gradients, fmt=["%.6f", "%.6f", "%.6f", "%d"])
    np.savetxt(bvec_fname, gradients[:, :-1].T, fmt="%.3f")
...
def test_dmri_data_average_dwi_model_estimation(
    ...
    monkeypatch.setattr(ants, "_run_registration", _identity_run_registration)
    ...
    # Monkeypatch clipping to avoid collapsing behaviors
    import nifreeze.data.filtering as filtering

    monkeypatch.setattr(filtering, "advanced_clip", _noop_advanced_clip)

    # Ignore warning due to absence of mask
    ...
    with context:
        if not brainmask_flag:
            warnings.filterwarnings(
                "ignore",
                message=MASK_ABSENCE_WARN_MSG,
    ...
    assert dwi_data.motion_affines is not None
    aff = np.asarray(dwi_data.motion_affines)
    assert aff.ndim == 3
    assert aff.shape[0] == len(dwi_data)
    assert aff.shape[1:] == (4, 4)
...
def test_dmri_data_dki_model_estimation(
    ...
    _, _, _, gradients, _b0_thres_fixture = setup_random_dwi_data
    bvals = gradients[..., -1]
    # ToDo
    # max_b is not used in the code
    # shellmask = np.ones_like(bvals, dtype=bool) if max_b is None else (bvals <= max_b)
    shellmask = np.ones_like(bvals, dtype=bool)
    gtab = gradient_table_from_bvals_bvecs(
        gradients[shellmask][..., -1], gradients[shellmask][..., :-1], b0_threshold=b0_thres
    )
    ...
    # The DWI object will only keep the non-zero bvals for its
    # gradients attribute. If we have 2 non-zero shells in the original
    # gradients, and bzero is None in the DWI dataset object, there
    # will not be enough bvals to fit the DKI model, so skip the test
    shell_count = len(unique_bvals_magnitude(dwi_data.bvals))
    if shell_count == 2 and dwi_data.bzero is None:
        pytest.skip(
            f"DKI requires 3 shells (including the zero-shell): found {shell_count} non-zero shells and bzero values are set to be ignored."
        )
...
def test_stacked_estimators(datadir, tmp_path, monkeypatch):
    ...
    dwi_motion = dmri.DWI.from_filename(
        datadir / "dmri_data" / "motion_test_data" / "dwi_motion.h5"
    )
    dwi_motion._filepath = tmp_path / "dwi_motion.h5"  # Prevent accidental overwriting
    dwi_motion.motion_affines = None  # Erase ground truth for estimation
