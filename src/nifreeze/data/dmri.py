...
class DWI(BaseDataset[np.ndarray | None]):
    ...
    def to_nifti(
        ...
        super().to_nifti(filename, order=order)
        ...
        if self.bzero is None or not insert_b0:
            ...
            # Open NIfTI generated by parent and prepend the b=0 volume
            nb.funcs.concat_images(
                (
                    nb.Nifti1Image(self.bzero, self.affine, self.datahdr),
                    *nb.funcs.four_to_three(nb.load(filename)),
                )
