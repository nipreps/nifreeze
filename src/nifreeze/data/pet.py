...
class PET(BaseDataset[np.ndarray]):
    ...
    def to_nifti(
        ...
        """
        Export the PET object to disk (NIfTI, temporal attribute files).
        ...
        return to_nifti(self, filename=filename, write_hmxfms=write_hmxfms, order=order)
...
def _compute_temporal_markers(frame_time: np.ndarray) -> Tuple[np.ndarray, float]:
    ...
    if len(frame_duration) == (len(frame_time) - 1):
        frame_duration = np.append(frame_duration, frame_duration[-1])  # last frame same as second-last
