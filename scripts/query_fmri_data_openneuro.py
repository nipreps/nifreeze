...
def query_snapshot_tree(dataset_id, snapshot_tag, tree=None, parent_path=""):
    ...
    for f in files:
        ...
        if f["directory"]:
            sub_files = query_snapshot_tree(dataset_id, snapshot_tag, f["id"], parent_path=current_path)
...
def filter_nonrelevant_files(files):
    ...
    bold_files = [
        f for f in files if re.search(r"_bold\.nii\.gz$", f["filename"])
    ]
    return bold_files
...
def install_dataset(ds_id, ds_path, snapshot_tag=None):
    ...
    if ds_path.exists():
        logging.info(
            f"Path already exists, loading existing dataset: {ds_path}")
        try:
            # Attempt to load the existing dataset
            dl_ds = dl.Dataset(ds_path)
            if not dl_ds.is_installed():
                logging.info(
                    f"Dataset at {ds_path} is not installed correctly, reinstalling...")
...
def filter_nonrelevant_datasets(datasets):
    return [ds for ds in datasets if "MRI" in ds["node"]["latestSnapshot"]["summary"]["modalities"]]
...
def process_dataset(ds, out_path):
    ...
    for f in bold_files:
        ...
        if n_vols and MIN_TIMEPOINTS <= n_vols <= MAX_TIMEPOINTS:
            ...
            logging.info(f"Not within required boundaries ({MIN_TIMEPOINTS}, {MAX_TIMEPOINTS}). Discarding run")
...
def _build_arg_parser():
    ...
    parser.add_argument(
        "seed", type=int, help="Random seed. Format (YYYYMMDD")
    parser.add_argument(
        "out_fname", type=Path, help="Output data list filename (*.TSV)")
    parser.add_argument(
        "out_ds_dirname", type=Path, help="Output dataset dirname")
    parser.add_argument(
        "--total_runs", type=int, default=TOTAL_RUNS, help="Number of total runs")
    parser.add_argument(
        "--contr_thr", type=float, default=INDIVIDUAL_DS_CONTR, help="Allowed contribution threshold for runs per dataset")
    parser.add_argument(
        "--min_timepoints_thr", type=int, default=MIN_TIMEPOINTS, help="Minimum number of BOLD timepoints per dataset")
    parser.add_argument(
        "--max_timepoints_thr", type=int, default=MAX_TIMEPOINTS, help="Maximum number BOLD timepoints per dataset")
    return parser
...
def main() -> None:
    ...
    if runs < TOTAL_RUNS:
        logging.info(f"{runs} total runs found. Please try querying more files to exceed the {TOTAL_RUNS} threshold.")
        return
