...
of features computed from the BOLD data files:
  - Criterion 1: no single dataset shall contribute more than 5% of the total
    number of runs.
  - Criterion 2: each BOLD run shall have between 300 and 1200 timepoints
    (inclusive).
"""
...
def identify_relevant_runs(
    ...
    Identifies the BOLD runs that fulfill the following criteria:
      - Criterion 1: the number of runs for a given dataset is below the
        threshold `int(contr_fraction * total_runs)`.
      - Criterion 2: the number of timepoints per BOLD run is between
       `[min_timepoints, max_timepoints]`.

    Runs are shuffled before the filtering process, and returns the first
    `total_runs` fulfilling the criteria.

    Parameters
    ----------
    df : :obj:`~pd.DataFrame`
        BOLD run information.
    contr_fraction : :obj:`float`
        Contribution fraction (%) over the total number of runs.
    min_timepoints : :obj:`int`
        Minimum number of time points.
    max_timepoints : :obj:`int``
        Maximum number of time points.
    total_runs : :obj:`int`
        Number of total runs to be kept.
    seed : :obj:`int`
        Random seed value.
    ...
    -------
    :obj:`~pd.DataFrame`
        First `total_runs` identified BOLD runs.
    """
